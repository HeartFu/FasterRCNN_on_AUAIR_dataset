...
class _ProposalTargetLayer(nn.Module):
    ...
    def __init__(self, nclasses, roi_batch_size=256, fg_graction=0.25,
                 bbox_normalize_targets_precomputed=True,
                 bbox_normalize_means=None,
                 bbox_normalize_stds=None,
                 fg_thresh=0.5,
                 bg_thresh_hi = 0.5, bg_thresh_lo=0.1):
        super(_ProposalTargetLayer, self).__init__()
        if bbox_normalize_means is None:
            bbox_normalize_means = [0.0, 0.0, 0.0, 0.0]
        if bbox_normalize_stds is None:
    ...
    def forward(self, all_rois, gt_boxes, num_boxes):
        ...
        num_images = 1
        rois_per_image = int(self.roi_batch_size / num_images)
        fg_rois_per_image = int(np.round(self.fg_graction * rois_per_image))
